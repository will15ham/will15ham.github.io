...
const getIpButton = document.querySelector("#js-tweet");

newQuote.addEventListener("click", getUA);
getIpButton.addEventListener("click", getIP);

function returnRandomUA() {
  // ripped from random github
  const UaArray = [
  ...
  return UaArray[Math.floor(Math.random() * UaArray.length)];
}

function getUA() {
  console.log("Clicked JS Button");
  const apiRoute = "https://httpbin.org/user-agent";
  fetch(apiRoute, {
    headers: {
      "User-Agent": returnRandomUA(),
    },
  })
    .then((response) => {
      if (!response.ok) {
...
function getIP() {
  console.log("Clicked JS Button");
  const apiRoute = "https://httpbin.org/ip";
  fetch(apiRoute, {
    headers: {
      "User-Agent": returnRandomUA(),
    },
...
function displayIp(quoteText) {
  const quoteElement = document.querySelector("#js-ip-text");
  quoteElement.textContent = `Request sent with IP: ${quoteText}`;
}

function displayQuote(quoteText) {
  const quoteElement = document.querySelector("#js-quote-text");
  quoteElement.textContent = `Request sent with UA: ${quoteText}`;
}
